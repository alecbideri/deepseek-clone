...
const ChatLabel = ({ openMenu, setOpenMenu, id, name }) => {
  ...
  const selectChat = () => {
    const chatData = chats.find((chat) => chat._id === id); // Fixed bug here
  ...
  return (
    ...
    >
      ...
      <div
        onClick={(e) => {
          e.stopPropagation(); // Changed to stopPropagation
      ...
      >
        ...
        />
        <div
          className={`absolute ${openMenu.id === id && openMenu.open ? "block" : "hidden"} -right-36 top-6 bg-gray-700 rounded-xl w-max p-2 z-10`}
        >
          <div
